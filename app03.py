...
# Setting the sidebar to collapsed
...
if selected_model == 'SARIMA':
    ...
    if len(train_X) > 0 and len(test_X) > 0:
        ...
        lstm_model.fit(train_X, train_y, epochs=10, batch_size=16, verbose=0)
        ...
        test_dates = data['Date'].iloc[train_size + seq_length:]

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=data["Date"], y=data[column], mode='lines', name='Actual', line=dict(color='blue')))
        fig.add_trace(go.Scatter(x=test_dates, y=test_predictions.flatten(), mode='lines', name='Predicted', line=dict(color='red')))
    ...
    prophet_data = data[['Date', column]].rename(columns={'Date': 'ds', column: 'y'})
